...
function get_latest_version() {
    ...
    curl -sL https://github.com/lottetian/hrp/releases/latest | grep '<title>Release' | cut -d" " -f4
}
...
function main() {
    ...
    if ! curl --output /dev/null --silent --head --fail "$url"; then
        ...
        pkg="hrp-$version-$os-$arch$pkg_suffix"
        url="https://github.com/lottetian/hrp/releases/download/$version/$pkg"
    fi
